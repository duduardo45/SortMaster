...
""" def convert_pixel_centroid_to_arduino(centroid):
    offset_x = 566
    over_limit_x = 2600
    image_x = 3840 - offset_x - (3840 - over_limit_x)
    image_y = 2160
    arduino_max_x = 1680
    arduino_max_y = 1800
    arduino_centroid_x = ((centroid[0] - offset_x) / image_x) * arduino_max_x
    arduino_centroid_y = (centroid[1] / image_y) * arduino_max_y
    return int(arduino_centroid_x), int(arduino_centroid_y) """

def convert_pixel_centroid_to_arduino(centroid):
    return int(1.025*centroid[0]-873.175), int(0.843*centroid[1]-5.139)

def is_in_box(centroid):
    _, arduino_y = convert_pixel_centroid_to_arduino(centroid)
    return arduino_y > 1200
...
def test_walk(pixel_x, pixel_y):
    ...
    time.sleep(15)
    position_x, position_y = convert_pixel_centroid_to_arduino((pixel_x, pixel_y))
    coord_str = f"anda {str(position_x).zfill(4)} {str(position_y).zfill(4)}"
    ser.write(coord_str.encode("utf-8"))
...
def process_image_objects(image_objects, captured_image_path):
    for obj in image_objects:
        print(f"Processing image for {obj['image_url']}...")
...
def process_polygon_objects(poly_objects, captured_image_path):
    for (color, shape), obj_list in poly_objects.items():
        ...
        centroids, file_paths = simple_poly.crop_to_largest_polygons(
            ...
            color,
            shape,
            os.path.join("output", "output_crop_{}.jpg"),
        )
...
def take_photo_command_arduino(objects: list[dict]):
    register_boxes(objects)
    captured_image_path = os.path.join(
        "detect_and_classify", "webcam", "webcam_photo.jpg"
    )
...
if __name__ == "__main__":
    """ capture_image_from_webcam(
        os.path.join("detect_and_classify", "webcam", "webcam_photo.jpg")
    ) """
    database_objects = [
        {
            "name": "Brazilian Real",
